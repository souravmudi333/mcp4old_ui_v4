...
async def get_my_permissions(
    team_id: Optional[str] = Query(None, description="Team context"),
    user=Depends(get_current_user_with_permissions),
    db: Session = Depends(get_db),
):
    try:
        ...
        # ✅ Detect platform admin from user payload (covers both common shapes)
        is_platform_admin = bool(
            user.get("is_admin") or
            user.get("user", {}).get("is_admin")
        )

        permissions = await permission_service.get_user_permissions(
            user_email=user["email"],
            team_id=team_id,
            is_platform_admin=is_platform_admin,  # ✅ new
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve your permissions",
        )


@router.get("/my/permissions/debug", response_model=Dict[str, Any])
async def get_my_permissions_debug(
  team_id: Optional[str] = Query(None, description="Team context"),
  user=Depends(get_current_user_with_permissions),
  db: Session = Depends(get_db),
):
  """
  Debug-friendly: returns user info + resolved permissions.
  Does NOT modify /my/permissions.
  """
  try:
    permission_service = PermissionService(db)
    ...
    logger.error(f"Failed to get my permissions debug for {user.get('email')}: {e}")
    raise HTTPException(
      status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
      detail="Failed to retrieve your permissions (debug)",
    )