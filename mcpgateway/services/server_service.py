...
class ServerPermissionError(ServerError):
    """Raised when a user is not allowed to edit a server."""
...
class ServerService:
    ...
    async def update_server(
        ...
        try:
            ...
            # --- Option B: Owner edit enforcement (and team-owner for team visibility) ---
            # Allow edit if user is the owner
            if getattr(server, "owner_email", None) != user_email:
                # If server is team-scoped, allow only team owners to edit
                if (server.visibility or "").lower() == "team":
                    if not server.team_id:
                        raise ValueError("Team-scoped server missing team_id for permission checking.")

                    # Use TeamManagementService (preferred) if available, otherwise fallback to direct membership query
                    try:
                        team_service = TeamManagementService(db)
                        role = await team_service.get_user_role_in_team(user_email, server.team_id)
                    except Exception:
                        role = None

                    if role != "owner":
                        raise ValueError("Only the owner (or team owner) can edit this server.")
                else:
                    raise ValueError("Only the owner can edit this server.")
            ...
            if server_update.visibility is not None:
                ...
                if new_visibility == "team":
                    ...
                    team_id = server.team_id or server_update.team_id

                    team = db.query(DbEmailTeam).filter(DbEmailTeam.id == team_id).first()
                    if not team:
                        raise ValueError(f"Team {team_id} not found")

                    # ✅ Allow edit if server owner OR team owner
                    if getattr(server, "owner_email", None) != user_email:
                        membership = (
                        ...
                        if not membership:
                            raise ValueError("Only the server owner or a team owner can edit this team server.")
    ...
    async def delete_server(
        ...
        """Permanently delete a server with guardrails.

        Rules:
        - Admin can delete anything.
        - Record owner can delete.
        - Team servers: team owner/admin OR record owner can delete.
        - Non-team servers: only owner OR admin can delete.
        """
        try:
            server = db.get(DbServer, server_id)
            if not server:
            ...
            if not actor_is_admin:
                if not actor_email:
                    raise ServerPermissionError("Delete not allowed: missing actor identity")

                owner_email = getattr(server, "owner_email", None)
                is_owner = bool(owner_email and owner_email.lower() == actor_email.lower())

                if is_owner:
                    pass
                else:
                    vis = (getattr(server, "visibility", None) or "private").lower()
                    ...
                    if vis == "team" and team_id:
                        ...
                        if role not in {"owner", "admin"}:
                            raise ServerPermissionError(
                                "Delete not allowed: only server owner or team owner/admin can delete this team server"
                            )
                    else:
                        raise ServerPermissionError(
                            "Delete not allowed: only server owner or admin can delete this server"
                        )
            # ---- end permission gate ----
        ...
        except (ServerNotFoundError, ServerPermissionError):
            raise
        except Exception as e:
            db.rollback()
