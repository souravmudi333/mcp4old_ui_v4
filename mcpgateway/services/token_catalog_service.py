...
class TokenCatalogService:
    ...
    def _clean_list(self, items: Optional[List[str]]) -> List[str]:
        """Normalize lists coming from UI/Swagger: remove empty strings."""
        if not items:
            return []
        cleaned: List[str] = []
        for x in items:
    ...
    def _clean_dict(self, data: Optional[dict]) -> dict:
        """Normalize placeholder dicts from Swagger like {'additionalProp1': {}} to {}."""
        if not data:
            return {}
        # Swagger UI placeholders often look like: {"additionalProp1": {}}
        if all(str(k).startswith("additionalProp") for k in data.keys()):
            if all((v == {} or v is None) for v in data.values()):
                return {}
        return data

    def _normalize_scope(self, scope: Optional["TokenScope"]) -> Optional["TokenScope"]:
        """Return a sanitized scope (no empty strings) so DB constraints don't break."""
        if not scope:
            return None

        scope.server_id = self._clean_str(scope.server_id)
        scope.permissions = self._clean_list(scope.permissions)
        scope.ip_restrictions = self._clean_list(scope.ip_restrictions)
        scope.time_restrictions = self._clean_dict(scope.time_restrictions)
        scope.usage_limits = self._clean_dict(scope.usage_limits)
    ...
    async def _generate_token(
        ...
        payload = {
            ...
            },  # Use actual admin status if user provided
    ...
    async def create_token(
        ...
        # Normalize UI/Swagger placeholder values so DB constraints don't break
        scope = self._normalize_scope(scope)
        tags = self._clean_list(tags or [])

        # Team token policy: require explicit permissions
        # (personal tokens may default to '*' but team tokens should be intentional)
        if team_id:
            if not scope or not scope.permissions:
                raise ValueError("Team token requires at least one permission in scope.permissions")
        ...
        raw_token = await self._generate_token(
    ...
    async def list_all_tokens(
        ...
        """List all tokens (admin use), optionally filtered by user_email/team_id."""
        if limit <= 0 or limit > 1000:
            limit = 100
        offset = max(offset, 0)
    ...
    async def update_token(
        ...
        # Normalize UI/Swagger placeholder values
        if tags is not None:
    ...
    async def revoke_token(self, token_id: str, revoked_by: str, reason: Optional[str] = None) -> bool:
        ...
        # Cache fields before delete (safer with ORM)
        token_jti = token.jti
        ...
        # Add to blacklist
        revocation = TokenRevocation(jti=token_jti, revoked_by=revoked_by, reason=reason)

        self.db.add(revocation)
        self.db.commit()

        logger.info(f"Revoked token '{token_name}' (JTI: {token_jti}) by {revoked_by}")
    ...
    async def create_token_admin_team(
        self,
        user_email: str,
        team_id: str,
        name: str,
        ...
        """
        Admin bypass: create a team token without requiring team owner membership.
        Keeps the same validation rules (team must exist, team token must have explicit permissions).
        """
        ...
        # duplicate name check scoped by (user_email, team_id)
        existing = self.db.execute(
        select(EmailApiToken).where(
            and_(
            EmailApiToken.user_email == user_email,
        ...
        if existing:
            raise ValueError(f"Token name '{name}' already exists for team {team_id}")
