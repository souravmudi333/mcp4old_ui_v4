...
class TeamInvitationService:
    ...
    async def create_invitation(self, team_id: str, email: str, role: str, invited_by: str, expiry_days: Optional[int] = None) -> Optional[EmailTeamInvitation]:
        ...
        try:
            ...
            inviter_user = self.db.query(EmailUser).filter(
                EmailUser.email == invited_by
            ).first()

            if inviter_user and inviter_user.is_admin:
                pass  # Admin override — allow

            else:
                inviter_membership = self.db.query(EmailTeamMember).filter(
                    EmailTeamMember.team_id == team_id,
                    EmailTeamMember.user_email == invited_by,
    ...
    async def revoke_invitation(self, invitation_id: str, revoked_by: str, is_admin: bool = False) -> bool:
        ...
        try:
            ...
            if not is_admin:
                revoker_membership = (
                    ...
                    .filter(
                        EmailTeamMember.team_id == invitation.team_id,
                        EmailTeamMember.user_email == revoked_by,
                        EmailTeamMember.is_active.is_(True)
                    )
                    .first()
                )
                ...
                if not revoker_membership or revoker_membership.role != "owner":
                    logger.warning(
                        f"User {revoked_by} does not have permission to revoke invitation {invitation_id}"
                    )
                    return False
