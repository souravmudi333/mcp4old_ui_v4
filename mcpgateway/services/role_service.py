...
class RoleService:
    ...
    async def delete_role(self, role_id: str) -> bool:
            ...
            # Deactivate all user assignments of this role (FIXED)
    ...
    async def assign_role_to_user(
        ...
        """Assign a role to a user (idempotent & safe)."""
        ...
        # Normalize & validate scope
        scope = scope.lower()
        if role.scope != scope:
            raise ValueError(
                f"Role scope '{role.scope}' doesn't match assignment scope '{scope}'"
            )

        if scope == "team" and not scope_id:
            raise ValueError("scope_id required for team-scoped assignments")

        if scope in {"global", "personal"}:
            scope_id = None

        # ---- IMPORTANT FIX: handle multiple existing assignments ----
        existing_assignments = await self.get_user_role_assignment(
        ...
        elif not isinstance(existing_assignments, list):
        ...
        active_assignments = [
            ...
            if getattr(r, "is_active", True) and not r.is_expired()
        ]
        ...
        logger.info(
            f"Assigned role {role.name} to {user_email} "
    ...
    async def revoke_role_from_user(self, user_email: str, role_id: str, scope: str, scope_id: Optional[str]) -> bool:
            ...
            Examples:
                ...
                True
            """
            # Fetch all matching role assignments instead of assuming only one
            user_roles = await self.get_user_role_assignment(user_email, role_id, scope, scope_id)

            # If the query returned None or an empty list, nothing to revoke
            if not user_roles:
                return False
