...
class GatewayPermissionError(GatewayError):
    """Raised when a user attempts to modify a gateway without being the owner."""
...
class GatewayService:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self) -> None:
        ...
        if self.redis_url and REDIS_AVAILABLE:
            ...
            temp_dir = os.path.dirname(__file__) # will store lock file in current script location # tempfile.gettempdir() - this is old # os.path.expanduser("~") - stores lock file in users home folder
    ...
    async def update_gateway(
        ...
        try:  # pylint: disable=too-many-nested-blocks
            ...
            # ---- Ownership / edit permission gate ----
            # Allowed:
            #   1) Admins
            #   2) Record owner (gateway.owner_email)
            #   3) Team Owner (if visibility is team and actor is team owner)
            if not actor_is_admin:
                if not actor_email:
                    raise GatewayPermissionError("Edit not allowed: missing actor identity")

                owner_email = getattr(gateway, "owner_email", None)
                if owner_email and owner_email == actor_email:
                    pass  # ✅ gateway creator/owner can edit
                else:
                    vis = (getattr(gateway, "visibility", None) or "private").lower()
                    ...
                    if vis == "team" and team_id:
                        ...
                        if role != "owner":
                            raise GatewayPermissionError("Edit not allowed: only gateway owner or team owner can edit this team gateway")
                    else:
                        raise GatewayPermissionError("Edit not allowed: only gateway owner can edit this gateway")
            # ---- end permission gate ----
            ...
            if gateway.enabled or include_inactive:
                ...
                if gateway_update.name is not None and gateway_update.name != gateway.name:
                    ...
                    vis = (gateway_update.visibility or gateway.visibility or "private").lower()
                    ...
                    if vis == "public":
                        ...
                        # prefer updated team_id if provided, else existing
                        team_id = gateway_update.team_id or gateway.team_id
                        if team_id:
                            existing_gateway = (
                                db.execute(
                                    select(DbGateway).where(
                ...
                if gateway_update.owner_email is not None:
                    gateway.owner_email = gateway_update.owner_email
                ...
                if gateway_update.passthrough_headers is not None:
                    ...
                    elif isinstance(gateway_update.passthrough_headers, str):
                        gateway.passthrough_headers = [h.strip() for h in gateway_update.passthrough_headers.split(",") if h.strip()]
                    else:
                ...
                # Auth type/value
                if getattr(gateway, "auth_type", None) is not None and gateway_update.auth_type is not None:
                ...
                if gateway_update.url is not None:
                    try:
                        capabilities, tools, resources, prompts = await self._initialize_gateway(
                            gateway.url, gateway.auth_value, gateway.transport, gateway.auth_type, gateway.oauth_config
                        )
                        new_tool_names = [t.name for t in tools]
                        new_resource_uris = [r.uri for r in resources]
                        new_prompt_names = [p.name for p in prompts]
                        ...
                        for resource in resources:
                            existing_resource = (
                                db.execute(select(DbResource).where(DbResource.uri == resource.uri).where(DbResource.gateway_id == gateway_id))
                                .scalar_one_or_none()
                            )
                        ...
                        for prompt in prompts:
                            existing_prompt = (
                                db.execute(select(DbPrompt).where(DbPrompt.name == prompt.name).where(DbPrompt.gateway_id == gateway_id))
                                .scalar_one_or_none()
                            )
                        ...
                        gateway.tools = [t for t in gateway.tools if t.original_name in new_tool_names]
                        gateway.resources = [r for r in gateway.resources if r.uri in new_resource_uris]
                        gateway.prompts = [p for p in gateway.prompts if p.name in new_prompt_names]
            ...
            raise
        except GatewayPermissionError as gpe:
            logger.error(f"GatewayPermissionError: {gpe}")
            raise
        except Exception as e:
            db.rollback()
    ...
    async def delete_gateway(
        ...
        """
        Delete a gateway by its ID with guardrails.

        Rules (same as tools):
        - Admin can delete anything.
        - Gateway owner can delete.
        - Team gateways: team owner/admin OR gateway owner can delete.
        - Non-team gateways: only owner OR admin can delete.
        """
        try:
            gateway = db.get(DbGateway, gateway_id)
            if not gateway:
            ...
            if not actor_is_admin:
                if not actor_email:
                    raise GatewayPermissionError("Delete not allowed: missing actor identity")
                    ...
                    if vis == "team" and team_id:
                        ...
                        if role not in {"owner", "admin"}:
                            raise GatewayPermissionError(
                                "Delete not allowed: only gateway owner or team owner/admin can delete this team gateway"
                            )
                    else:
                        raise GatewayPermissionError(
                            "Delete not allowed: only gateway owner or admin can delete this gateway"
                        )
            # ---- end permission gate ----
        ...
        except (GatewayNotFoundError, GatewayPermissionError):
            raise
        except Exception as e:
            db.rollback()
