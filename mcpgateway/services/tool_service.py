...
class ToolPermissionError(ToolError):
    """Raised when a user is not authorized to perform an operation on a tool."""
...
class ToolService:
    ...
    def _convert_tool_to_read(self, tool: DbTool) -> ToolRead:
        ...
        # ---- Safe auth handling (never crash listing due to bad auth_value) ----
        tool_dict["auth"] = None

        auth_type = (getattr(tool, "auth_type", None) or "").strip().lower()
        auth_value = getattr(tool, "auth_value", None)

        decoded_auth_value = None
        if auth_type and auth_value:
            try:
                decoded_auth_value = decode_auth(auth_value)
            except Exception as ex:
                logger.warning(
                    "Auth decode failed for tool_id=%s auth_type=%s. Skipping auth. err=%s",
        ...
        # Masked auth response only if decode succeeded
        if decoded_auth_value and auth_type == "basic":
            try:
                auth_header = decoded_auth_value.get("Authorization", "")
                if auth_header.startswith("Basic "):
                    decoded_bytes = base64.b64decode(auth_header.split("Basic ", 1)[1])
                    raw = decoded_bytes.decode("utf-8", errors="replace")
                    if ":" in raw:
                    ...
                    tool_dict["auth"] = {"auth_type": "basic", "username": None, "password": None}
            except (binascii.Error, UnicodeDecodeError, ValueError) as ex:
                logger.warning(
                    "Failed to parse BASIC auth for tool_id=%s. err=%s",
                    getattr(tool, "id", None),
                    str(ex),
                )
                tool_dict["auth"] = {"auth_type": "basic", "username": None, "password": None}

        elif decoded_auth_value and auth_type == "bearer":
            token = decoded_auth_value.get("Authorization", "")
            tool_dict["auth"] = {
                "auth_type": "bearer",
                "token": "********" if token else None,
            }

        elif decoded_auth_value and auth_type == "authheaders":
            try:
                if isinstance(decoded_auth_value, dict) and decoded_auth_value:
                    k = next(iter(decoded_auth_value))
                    v = decoded_auth_value.get(k)
                    tool_dict["auth"] = {
                        "auth_type": "authheaders",
                        "auth_header_key": k,
                        "auth_header_value": "********" if v else None,
                    }
                else:
                    tool_dict["auth"] = {"auth_type": "authheaders", "auth_header_key": None, "auth_header_value": None}
            except Exception as ex:
                logger.warning(
                    "Failed to parse authheaders for tool_id=%s. err=%s",
                    getattr(tool, "id", None),
                    str(ex),
                )
                tool_dict["auth"] = {"auth_type": "authheaders", "auth_header_key": None, "auth_header_value": None}

        # ---- Names / display fields ----
    ...
    async def delete_tool(
        ...
        """
        Delete a tool by its ID with guardrails.

        Rules:
        - Admin can delete anything.
        - Tool owner can delete.
        - Team tools: team owner/admin OR tool owner can delete.
        - Non-team tools: only owner OR admin can delete.
        - If user has tools.delete but is not permitted, raise ToolPermissionError
            with a clear reason (used by UI).
        """
        try:
            tool = db.get(DbTool, tool_id)
            if not tool:
            ...
            if not actor_is_admin:
                if not actor_email:
                    raise ToolPermissionError("Delete not allowed: missing actor identity")

                owner_email = getattr(tool, "owner_email", None)
                is_owner = bool(owner_email and owner_email.lower() == actor_email.lower())

                if is_owner:
                    pass  # tool owner can delete
                else:
                    vis = (getattr(tool, "visibility", None) or "private").lower()
                    team_id = getattr(tool, "team_id", None)

                    if vis == "team" and team_id:
                        team_service = TeamManagementService(db)
                        role = await team_service.get_user_role_in_team(actor_email, team_id)

                        # Allow team owner/admin (and tool owner already handled above)
                        if role not in {"owner", "admin"}:
                            # Your requirement #6/#8: explain why
                            raise ToolPermissionError(
                                "Delete not allowed: only tool owner or team owner/admin can delete this team tool"
                            )
                    else:
                        # Private/public (non-team) and not owner
                        raise ToolPermissionError(
                            "Delete not allowed: only tool owner or admin can delete this tool"
                        )
            # ---- end permission gate ----
    ...
    async def update_tool(
        ...
        try:
            ...
            # -------------------------
            actor_email = modified_by  # UI/API caller identity (email)

            # Platform admin can always edit
            if not actor_is_admin:
                effective_visibility = (getattr(tool_update, "visibility", None) or getattr(tool, "visibility", None) or "private").lower()

                # For private/public tools: only owner can edit
                if effective_visibility in {"private", "public"}:
                    if not actor_email or tool.owner_email != actor_email:
                        raise ToolPermissionError("Not authorized to edit this tool. Only the owner or an admin can edit it.")

                # For team tools: owner/admin of team OR tool owner (must be team member)
                elif effective_visibility == "team":
                    if not tool.team_id:
                        raise ToolPermissionError("Team tool is missing team_id. Cannot authorize edit.")

                    team_service = TeamManagementService(db)
                    role = await team_service.get_user_role_in_team(actor_email or "", str(tool.team_id))

                    # Team admin/owner can edit any team tool
                    if role in {"owner", "admin"}:
                        pass
                    else:
                        # Tool owner can edit if they are a member of the team
                        if not actor_email or tool.owner_email != actor_email or role is None:
                            raise ToolPermissionError("Not authorized to edit this team tool. Only team admins/owners, the tool owner, or an admin can edit it.")
                else:
                    # Unknown visibility -> default deny (safe)
                    raise ToolPermissionError("Invalid tool visibility. Cannot authorize edit.")
            # -------------------------
            # End Authorization Gate
            # -------------------------

            # Prevent ownership/team spoofing via update payload (keep transfers as separate ops)
            if hasattr(tool_update, "owner_email"):
                try:
                    setattr(tool_update, "owner_email", None)
                except Exception:
                    pass
            if hasattr(tool_update, "team_id"):
                try:
                    setattr(tool_update, "team_id", None)
                except Exception:
                    pass

            # Check for name change and ensure uniqueness
            if tool_update.name and tool_update.name != tool.name:
                # Use effective visibility (avoid .lower() crash when None)
                effective_visibility = (getattr(tool_update, "visibility", None) or getattr(tool, "visibility", None) or "private").lower()
                ...
                if effective_visibility == "public":
                    # Check for existing public tool with the same name
                    existing_tool = db.execute(
                        select(DbTool).where(DbTool.custom_name == tool_update.custom_name, DbTool.visibility == "public")
                    ).scalar_one_or_none()
                    if existing_tool:
                    ...
                    # Only enforce if we have a team_id on the existing tool
                    if tool.team_id:
                        existing_tool = db.execute(
                            select(DbTool).where(
                                DbTool.custom_name == tool_update.custom_name,
                                DbTool.visibility == "team",
                                DbTool.team_id == tool.team_id,
                            )
                        ).scalar_one_or_none()
                        if existing_tool:
                            raise ToolNameConflictError(existing_tool.custom_name, enabled=existing_tool.enabled, tool_id=existing_tool.id, visibility=existing_tool.visibility)
            ...
            raise tnce
        except ToolPermissionError as tpe:
            # No DB writes happened before this point, so rollback is safe but not strictly required
            db.rollback()
            logger.warning(f"Tool permission denied for tool {tool_id}: {tpe}")
            raise tpe
        except Exception as ex:
            db.rollback()
